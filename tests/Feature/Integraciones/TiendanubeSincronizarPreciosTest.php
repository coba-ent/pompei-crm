...
/**
 * US7 (spec 018 ampliación): "Sincronizar precios ahora" — contracts §1a.
 * No concurrencia (FR-036), los tres cortes de FR-032/FR-033 con un único
 * registro, y 409 con motivo cuando no hay Lista de Precios configurada.
 */
class TiendanubeSincronizarPreciosTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        return TiendanubeOperacionLog::where('operacion', 'sincronizar_precio')->where('resultado', 'bloqueada')->count();
    }
    ...
    public function test_rechazo_de_un_vinculo_no_interrumpe_otros_envios_de_precio(): void
    {
        ...
        Http::fake([
            'admin-mcp.tiendanube.com/' => Http::sequence()
                ...
                ->push([
                    'jsonrpc' => '2.0', 'id' => 2,
                    'result' => ['isError' => true, 'content' => [['type' => 'text', 'text' => 'Variante despublicada']]],
                ], 200),
        ]);
        ...
        $this->assertSame('Variante despublicada', $vinculoError->fresh()->precio_error);
    }
}
