...
- [ ] T001 Crear los directorios `app/Http/Controllers/Ingresos/` (si no existe tras la spec 012),
  `app/Services/Tiendanube/` y `resources/views/ingresos/tiendanube/`, siguiendo plan.md §Project
  Structure
- [ ] T002 [P] Confirmar que no hace falta ninguna dependencia nueva: `Http`, `Cache::lock`, DataTables,
...
- [ ] T003 [P] Migración `create_tn_ordenes_table` con todos los campos e índices de data-model.md §1,
  incluidos los **únicos** en `tn_order_id` y `venta_id` (FR-013, FR-032b)
- [ ] T004 [P] Migración `create_tn_orden_items_table` per data-model.md §2
- [ ] T005 [P] Migración `create_tn_variante_producto_table` con los **dos índices únicos** que
  garantizan la cardinalidad 1:1 (FR-022)
- [ ] T006 [P] Migración `add_ventas_fields_to_tn_configuracion_table` con las columnas de
  data-model.md §4 (incluida `cuenta_tesoreria_id`, nueva respecto de Mercado Libre — research.md R5)
- [ ] T007 [P] Migración `add_tn_customer_id_to_clientes_table` con índice, mismo patrón que
  `add_ml_user_id_to_clientes_table` (spec 012) — FR-036/036a
- [ ] T008 [P] Migración `add_tiendanube_to_origen_enum_ventas_table` (o el mecanismo ya usado por la
  spec 012 para el enum `origen`) agregando el valor `tiendanube` (FR-035)

### 2.b Enums y modelos

- [ ] T009 [P] Crear `app/Enums/Tiendanube/EstadoConversion.php` con los cinco estados y transiciones
  válidas de data-model.md §1 — **no** reutilizar el enum de Mercado Libre (research.md R6)
- [ ] T010 [P] Crear `app/Enums/Tiendanube/MotivoRequiereAtencion.php` con los motivos propios de
...
- [ ] T011 [P] Crear `app/Models/Integraciones/TiendanubeOrden.php` con relaciones, casts y scopes por
  estado de conversión
- [ ] T012 [P] Crear `app/Models/Integraciones/TiendanubeOrdenItem.php`
- [ ] T013 [P] Crear `app/Models/Integraciones/TiendanubeVarianteProducto.php` con la relación a
  Producto
- [ ] T014 Extender `app/Models/Integraciones/TiendanubeConfiguracion.php` (spec 015) con los campos
  nuevos, sus casts, y `depositoEfectivo()` calcado de `MercadoLibreConfiguracion::depositoEfectivo()`
  (research.md R4, FR-047)
- [ ] T015 [P] Extender `app/Models/Venta.php` con `tiendanube` en el scope/enum de `origen` existente
  (FR-035)

### 2.c Cliente de API y traducción

- [ ] T016 Crear `app/Services/Tiendanube/TraductorOrdenes.php` como único punto que interpreta el
  formato de Tiendanube, mapeando `status`+`payment_status` a `EstadoConversion` según la tabla de
  FR-007a (research.md R1)
- [ ] T017 Implementar en `TraductorOrdenes` el descarte explícito de órdenes `storefront=meli` como
...
- [ ] T018 [P] Test de `TraductorOrdenes` en `tests/Feature/Integraciones/TiendanubeTraductorOrdenesTest.php`:
  ...
  (FR-012a, SC-010)
- [ ] T019 [P] Extender `app/Services/Ingresos/StockDeVenta.php::resolverDeposito()` con la rama
  `$venta->origen === 'tiendanube' → TiendanubeConfiguracion::actual()->depositoEfectivo()` (plan.md §5,
  FR-047)
- [ ] T020 [P] Test de regresión en `tests/Feature/Ingresos/VentaStockTest.php` (ya existente, spec 012):
...
- [ ] T021 [US1] Crear `app/Services/Tiendanube/SincronizadorOrdenes.php`: consulta `GET /orders` con
  `updated_at_min`/`created_at_min` incremental desde la última marca exitosa, `channels` excluyendo
  `meli` (research.md R2), paginación, *upsert* por `tn_order_id` (FR-012, FR-013, FR-016)
- [ ] T022 [US1] Implementar en `SincronizadorOrdenes` el candado propio con `Cache::lock`, independiente
  del de Mercado Libre (FR-014)
- [ ] T023 [US1] Implementar los cortes previos a paginar: función "Tiendanube" desactivada, modo sólo
  lectura, conexión caída o no configurada, con un único registro en `tn_operaciones_log` (FR-017,
  FR-018)
- [ ] T024 [US1] Implementar persistencia de avance por página para retomar sin reprocesar ante una
  interrupción (FR-015)
- [ ] T025 [US1] Implementar espera creciente ante 429 (leaky bucket ~2/seg, ráfaga 40) y reintento
  acotado ante fallas temporales (FR-020)
- [ ] T025a [US1] Registrar **cada** corrida de sincronización (éxito, error, bloqueada) en
  `tn_operaciones_log`, no sólo los intentos bloqueados de T023 — mismo criterio que
  `ml_operaciones_log` en la spec 012 (FR-019)
- [ ] T026 [US1] Crear `app/Http/Controllers/Ingresos/TiendanubeVentaController.php` con `index`,
  `datatable` y `sincronizar` según contracts §1
- [ ] T027 [US1] Registrar las rutas del grupo Ingresos → Tiendanube en `routes/web.php` con el permiso
  `ventas.ver` y el guard de función activa (FR-002, FR-003)
- [ ] T028 [US1] Crear la vista del listado en `resources/views/ingresos/tiendanube/index.blade.php`
  extendiendo `layouts.default`, con columnas `status`/`payment_status`/`shipping_status` (FR-005)
- [ ] T028a [US1] Distinguir visualmente en el listado las órdenes en "Requiere atención", mostrando el
...
- [ ] T029 [US1] Crear `resources/js/tiendanube-ventas.js` con la DataTable server-side, panel de
  filtros y botón "Sincronizar ahora" con Toastr, sin recarga de página (FR-004, FR-006, FR-009)
- [ ] T030 [US1] Registrar el bundle nuevo en `vite.config.js` y el pagelevel en `config/dz.php`
- [ ] T031 [US1] Agregar la entrada condicional "Tiendanube" al menú Ingresos en
  `resources/views/elements/sidebar.blade.php`, visible sólo con la función activa (FR-002)
- [ ] T032 [P] [US1] Test en `tests/Feature/Integraciones/TiendanubeSincronizacionTest.php`:
...
- [ ] T033 [P] [US1] Test de los tres cortes de bloqueo y del acceso denegado sin permiso (FR-017,
  FR-018, FR-003)
...
- [ ] T034 [US2] Crear `app/Http/Controllers/Ingresos/TiendanubeVinculacionController.php` con `index`,
  `datatable`, `store`, `update` y `destroy` según contracts §2
- [ ] T035 [US2] Crear el FormRequest de vinculación validando la cardinalidad 1:1 con mensajes por
  campo (FR-022)
- [ ] T036 [US2] Implementar en `destroy` la advertencia por órdenes ya convertidas, sin modificar las
  Ventas existentes (FR-026, FR-062)
- [ ] T037 [US2] Registrar las rutas de vinculación en `routes/web.php`
- [ ] T038 [US2] Crear la vista `resources/views/ingresos/tiendanube/vinculaciones.blade.php` con
  DataTable server-side (FR-024, FR-025)
- [ ] T039 [P] [US2] Test en `tests/Feature/Integraciones/TiendanubeVinculacionTest.php`: crear, editar,
...
- [ ] T040 [US3] Crear `app/Services/Tiendanube/ResolutorCliente.php`: emparejar por `tn_customer_id`,
...
- [ ] T041 [US3] Implementar en `ResolutorCliente` la derivación del tipo de comprobante: primero la
  condición de IVA ya cargada en el Cliente emparejado; si no la tiene, aproximar por
  `billing_document_type` según la tabla de FR-040 (CUIT→A, cualquier otro valor o ausencia→B) —
  FR-039/FR-040/FR-040a/FR-040d
- [ ] T041a [US3] Implementar en `ResolutorCliente` que **no** se sobrescriba la condición de IVA ni los
...
- [ ] T042 [US3] Crear `app/Services/Tiendanube/ConversorOrdenAVenta.php`: candado por orden
  (`Cache::lock`), revalidación, transacción única que crea Venta+ítems+cobranza+stock (FR-032a, FR-048)
- [ ] T043 [US3] Implementar en `ConversorOrdenAVenta` la desagregación de IVA reutilizando el mismo
  mecanismo que Mercado Libre (FR-030a) sin descuento general ni conceptos extra (FR-030c), rechazando
  moneda distinta (FR-030d)
- [ ] T044 [US3] Implementar la resolución de la cuenta de Tesorería **configurada** por FK (no por
  nombre fijo, research.md R5) y el rechazo con motivo claro si no existe o está inactiva (FR-045,
  FR-045a)
- [ ] T045 [US3] Implementar el respaldo de unicidad orden→Venta a nivel de datos sobre `tn_ordenes.venta_id`
  (ya cubierto por el índice de T003, FR-032b) y el manejo del `QueryException` de carrera (FR-032a)
- [ ] T046 [US3] Agregar las acciones `convertir`/`convertirGuardar` a `TiendanubeVentaController`
  reutilizando el formulario de página completa "Nueva Venta" precargado (contracts §1, FR-028, FR-029)
- [ ] T046a [US3] Implementar en el formulario de conversión el selector con buscador para vincular
...
- [ ] T047 [US3] Registrar las rutas de conversión en `routes/web.php`
- [ ] T048 [US3] Extender el listado de Ventas: columna/filtro "Creada Desde" agrega "Tiendanube"
  (FR-035a) — **no** crear un filtro separado
- [ ] T049 [P] [US3] Test en `tests/Feature/Integraciones/TiendanubeConversionTest.php`: conversión
...
- [ ] T050 [P] [US3] Test de derivación de comprobante en
  `tests/Feature/Integraciones/TiendanubeComprobanteTest.php`: CUIT→A, DNI/otro/sin dato→B, Cliente ya
  con condición de IVA cargada usa esa condición en vez de aproximar (FR-039, FR-040, SC-015)
- [ ] T051 [P] [US3] Test de precondiciones: variante sin vincular, cliente ambiguo, moneda distinta,
...
- [ ] T052 [US4] Extender `TiendanubeConfiguracionController` (spec 015) con el endpoint
...
- [ ] T053 [US4] Crear el FormRequest de validación, con `cuenta_tesoreria_id` **nullable pero
  bloqueante al convertir** (FR-045a) — distinto de `deposito_id`/`categoria_venta_id`, que sí tienen
  fallback o ausencia tolerada
- [ ] T054 [US4] Extender la vista de configuración de Tiendanube (spec 015) con los selectores nuevos
...
- [ ] T055 [US4] Crear `app/Console/Commands/SincronizarOrdenesTiendanube.php`, mismo mecanismo de
  portabilidad hosting-compartido/VPS que `mercadolibre:sincronizar-ordenes` (FR-011)
- [ ] T056 [P] [US4] Test de configuración: persistencia de cada campo, bloqueo de conversión sin cuenta
...
- [ ] T057 [US5] Implementar en `SincronizadorOrdenes` la delegación a `ConversorOrdenAVenta` cuando
  `creacion_automatica` está activo, por cada orden en estado "Lista para convertir" (FR-051)
- [ ] T058 [US5] Implementar el marcado "Requiere atención" con motivo concreto cuando la orden no sea
  resoluble automáticamente, sin descontar stock (FR-052, FR-058 no aplica acá — ver US6)
- [ ] T059 [US5] Registrar en la Venta creada automáticamente el indicador y momento de creación
  automática (FR-054)
- [ ] T060 [US5] Implementar el manejo de fallo durante la creación automática: orden marcada con
  motivo, error registrado, sin Venta parcial (FR-055)
- [ ] T061 [P] [US5] Test en `tests/Feature/Integraciones/TiendanubeCreacionAutomaticaTest.php`:
...
- [ ] T062 [US6] Implementar en `SincronizadorOrdenes`/`TraductorOrdenes` la detección de `status`
...
- [ ] T063 [US6] Mostrar en el listado, de forma destacada, la orden convertida cuyo estado cambió
  después, con acceso a la Venta (US6, Acceptance Scenarios)
- [ ] T064 [US6] Deshabilitar la conversión de una orden cancelada antes de convertirse (FR-059)
- [ ] T065 [P] [US6] Test en `tests/Feature/Integraciones/TiendanubeCancelacionesTest.php`: orden
...
- [ ] T066 [P] Correr la suite completa de `tests/Feature/Integraciones/` (specs 011-013, 015, 016) y
  confirmar que sigue en verde — regresión mínima (quickstart.md §Regresión mínima)
- [ ] T067 [P] Actualizar `CREDENCIALES_ACCESO.txt` si el testing manual requirió datos de acceso nuevos
  (CLAUDE.md, regla de credenciales)
- [ ] T068 Ejecutar `quickstart.md` end-to-end contra el entorno local (migraciones, build de assets,
