...
- No se requiere un historial/auditoría de cambios de cobranzas en el alcance de esta feature (no se pidió explícitamente); si se corrige un dato, no queda rastro de cuál era el valor anterior más allá de los logs estándar de la aplicación. **Actualización 07/08/2026**: el módulo transversal de Auditoría (spec 054, `specs/054-auditoria-operaciones/`) sí registra automáticamente cada edición de cobranza (y la actualización del Movimiento de Tesorería asociado) como evento de "Modificó" — sin guardar el valor anterior campo por campo, pero sí quién/cuándo editó. Esta feature (053) no necesita implementar nada propio para eso: lo cubre el Observer genérico de Cobro de la spec 054.
