...
(function () {
    ...
    function inicializarTransformarTodasEnVenta() {
        const $btn = $('#btn-transformar-todas-en-venta-tn');
        if (!$btn.length) { return; }

        const modalEl = document.getElementById('modal-resultado-transformar-venta-tn');
            ...
            $.ajax({ url: rutas.transformarTodasEnVenta, method: 'POST' })
                .done((resp) => {
                    ...
                    if (resp.fallidas > 0 && modal) {
                        $('#resultado-transformar-venta-tn-body').html(renderResultadoTransformarEnVenta(resp));
                        modal.show();
                    }

                    if (resp.convertidas > 0 && window._tnOrdenesTabla) { window._tnOrdenesTabla.ajax.reload(null, false); }
                })
                .fail((xhr) => {
                    const resp = xhr.responseJSON || {};
