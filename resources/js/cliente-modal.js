/**
 * Modal de alta/edición de Cliente (ficha completa — mismo `_modal_form` de
 * Clientes) reutilizable desde selects de otras pantallas (Presupuestos,
 * Ventas): "Crear Cliente" y el lápiz de cada fila abren ESTE modal, no uno
 * simplificado de sólo nombre.
 *
 * Requiere que la vista incluya `clientes._modal_form` y llame a
 * `window.ClienteModal.init(rutas)` con { store, show, localidades, verificarDocumento }
 * antes de invocar `crear()`/`editar()`.
 */
(function () {
    'use strict';
    ...
    const $ = window.jQuery;
    if (!$) {
        console.error('[cliente-modal] jQuery no está disponible.');
        return;
    }
    ...
    function toast(tipo, mensaje, titulo) {
        if (window.toastr && window.toastr[tipo]) {
            ...
            console.log('[cliente-modal][' + tipo + ']', mensaje);
        }
    }
    ...
    function precargarLocalidades(cliente) {
        $form.find('.js-provincia').each(function () {
            ...
            if (provincia) { cargarLocalidades($loc, provincia, cliente[target]); }
        });
    }
        ...
        $form.on('submit', function (e) {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    ...
                    if (typeof onGuardadoActual === 'function') { onGuardadoActual(resp.cliente); }
                })
                .fail(function (xhr) {
                ...
                .always(function () {
                    if (window.AppBtn) { window.AppBtn.loading('#btn-guardar-cliente', false); }
                });
        });
    }
    ...
    function editar(id, onGuardado, soloLectura, titulo) {
        ...
        $.getJSON(rutas.show + '/' + id)
            .done(function (resp) {
                ...
                (c.contactos || []).forEach(function (item) { agregarContacto(item); });
                ...
                abrirModal(titulo || (soloLectura ? 'Ver Cliente' : 'Editar Cliente'));
            })
            .fail(function () { toast('error', 'No se pudo cargar el cliente.'); });
    }

    window.ClienteModal = { init: init, crear: crear, editar: editar };
})();
